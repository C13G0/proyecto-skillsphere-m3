...
export const OrganicGradientBackground = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Shaders nativos para renderizar las ondas de color fluidas
    const vertexShader = `
      varying vec2 vUv;
      void main() {
        vUv = uv;
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Paleta de colores orgánicos modificada sutilmente para ambiente oscuro
        vec3 color1 = vec3(0.9, 0.2, 0.05);  // Naranja intenso
        vec3 color2 = vec3(0.0, 0.7, 0.85);  // Neón cian
        vec3 color3 = vec3(0.5, 0.05, 0.7);  // Púrpura profundo
        vec3 color4 = vec3(0.05, 0.75, 0.3); // Verde SkillSphere
        vec3 color5 = vec3(0.9, 0.6, 0.0);   // Dorado sutil
        
        vec3 finalColor = vec3(0.01, 0.02, 0.04); // Base azul-oscura profunda
        ...
        finalColor += (microTurbulence * noiseDetail) * 0.4;
        
        finalColor = pow(finalColor, vec3(0.95));
        finalColor *= 1.1;
        
        float vignette = 1.0 - length(uv - 0.5) * 1.5;
        finalColor *= smoothstep(0.1, 1.0, vignette);
    ...
    // Línea de tiempo de animación fluida con GSAP
    ...
    const handleMouseMove = (event) => {
      ...
      material.uniforms.u_turbulence.value += mouseInfluence * 0.05;
    };
    window.addEventListener('mousemove', handleMouseMove);

    // Bucle de animación de frames
    const animate = () => {
      timeRef.current += 0.005; // Movimiento lento y sumamente elegante
      material.uniforms.u_time.value = timeRef.current;
      material.uniforms.u_sharpness.value += Math.sin(timeRef.current * 0.5) * 0.05;
  ...
  return (
    <div style={{ position: 'relative', width: '100%', minHeight: '100vh' }}>
      {/* El canvas se posiciona "fixed" atrás de todo para no romper el scroll del grid */}
      <div 
        ref={mountRef} 
        style={{ 
          position: 'fixed', 
      ...
      />
      {/* Capa de cristal oscuro superpuesta que ecualiza los tonos del shader */}
      <div 
        style={{
          position: 'fixed',
          top: 0,
          ...
          width: '100vw',
          height: '100vh',
          backgroundColor: 'rgba(9, 13, 22, 0.65)', /* Oscurece el shader lo necesario */
          backdropFilter: 'blur(40px)',               /* Difumina las ondas orgánicas */
          zIndex: 1,
          pointerEvents: 'none'
        }}
      />
      {/* Contenedor real de la app donde se renderizan las tarjetas */}
